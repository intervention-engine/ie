...
func NewHuddleScheduler(config *HuddleConfig, s ie.Services) *HuddleScheduler {
...
func (hs *HuddleScheduler) createHuddles() error {
	...
	huddles := make([]*ie.Huddle, 0, hs.Config.LookAhead)
	checkRollOversAndEvents := true
	for t := today(); len(huddles) < hs.Config.LookAhead; t = t.AddDate(0, 0, 1) {
		...
		var originalMembers []ie.HuddleMembership
		if huddle == nil {
			// Create a new huddle

			huddle = ie.Huddle{Date: t} // NewHuddle(hs.Config.Name, hs.Config.LeaderID, t)
		} else {
...
func (hs *HuddleScheduler) findExistingHuddle(date time.Time) (*ie.Huddle, error) {
	searcher := search.NewMongoSearcher(server.Database)
	...
	if err := searcher.CreateQuery(search.Query{Resource: "Group", Query: queryStr}).All(&huddles); err != nil {
		...
		huddle := ie.Huddle(*huddles[0])
		return &huddle, nil
	}
	return nil, nil
}

func (hs *HuddleScheduler) addMembersBasedOnRiskScores(huddle *ie.Huddle, huddleIdx, targetSize int) {
...
func (hs *HuddleScheduler) addMembersBasedOnRecentEncounters(huddle *ie.Huddle, huddleIdx int) error {
			...
			for _, code := range eventConfig.TypeCodes {
				if codeMatches(result.Type, &code) {
					if d, matches := dateMatches(result.Period, &code, lowInclDate, highExclDate); matches {
						...
						var huddles []*ie.Huddle
						for i := range result.Huddles {
							h := ie.Huddle(result.Huddles[i])
...
func isPatientScheduledForSpecificEncounterReason(huddles []*ie.Huddle, patientID string, encounterDate time.Time) bool {
...
func (hs *HuddleScheduler) addMembersBasedOnRollOvers(huddle *ie.Huddle) {
	if hs.Config.RollOverDelayInDays <= 0 {
		return
	}
...
func getStringDate(huddle *ie.Huddle) string {
